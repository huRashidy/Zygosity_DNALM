...
def get_model_by_name(name, labels=None):
    if name != "XGB":
        raise ValueError("This script now supports only 'XGB'.")
...
def filter_features(X, var_thresh=0.001, corr_thresh=0.9):
    """
    Apply variance and correlation-based feature filtering.
    
    Args:
        X: numpy array of shape (n_samples, n_features)
        var_thresh: minimum variance threshold to keep a feature
        corr_thresh: maximum allowed absolute correlation between features
    
    Returns:
        Filtered numpy array of shape (n_samples, n_filtered_features)
    """
    ...
    df = df.loc[:, variances > var_thresh]
    print(f"After variance filtering: {df.shape} , with varthre={var_thresh}")
    ...
    print(f"After correlation filtering: {df.shape}")
    sys.stdout.flush()
...
def main_cv(X_dict, y, chr_name, method="XGB", filtering=False, var_thresh=0.0001, corr_thresh=0.9, n_splits=5):
    results = {}
    ...
    for name, embedding_tuple in X_dict.items():
        ...
        if filtering:
            X, X_mat, X_pat = [
                filter_features(emb, var_thresh=var_thresh, corr_thresh=corr_thresh)
                for emb in (X, X_mat, X_pat)
            ]
...
def main_refined(chromosome, model_name, data_dir="./data", results_dir="./data/results"):
    ...
    for emb in emb_types:
        ...
        vec_all = np.array([e.reshape(-1) for e in ind_all], dtype=np.float32)
        vec_mat = np.array([e.reshape(-1) for e in ind_mat], dtype=np.float32)
        vec_pat = np.array([e.reshape(-1) for e in ind_pat], dtype=np.float32)
        ...
        results_emb = main_cv(X_dict, labels_all, str(chromosome), method=model_name,
                              filtering=True, var_thresh=1e-5, corr_thresh=1, n_splits=5)
